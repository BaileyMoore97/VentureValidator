...
const CategoryInput = ({ categoryName, categoryColor, categoryIcon, onAddItem }: CategoryInputProps) => {
  ...
  const getAllSuggestions = (category: string) => {
    const allSuggestions: Record<string, string[]> = {
      "Food": ["Milk", "Bread", "Eggs", "Bananas", "Chicken", "Rice", "Pasta", "Tomatoes", "Onions", "Cheese", "Yogurt", "Apples", "Potatoes", "Ground Beef", "Olive Oil"],
      "Cleaning": ["Dish Soap", "Paper Towels", "Toilet Paper", "All-Purpose Cleaner", "Laundry Detergent", "Glass Cleaner", "Disinfectant", "Sponges", "Trash Bags", "Fabric Softener"],
      "Pet Goods": ["Dog Food", "Cat Litter", "Pet Treats", "Flea Shampoo", "Pet Toys", "Pet Bed", "Leash", "Food Bowls", "Litter Box", "Pet Carrier"],
      "Personal Care": ["Toothpaste", "Shampoo", "Deodorant", "Body Wash", "Razor", "Soap", "Conditioner", "Lotion", "Toothbrush", "Face Wash"],
      "Household Items": ["Light Bulbs", "Batteries", "Trash Bags", "Air Freshener", "Storage Containers", "Paper Plates", "Aluminum Foil", "Candles", "Extension Cord", "Duct Tape"],
      "Electronics": ["Phone Charger", "USB Cable", "Headphones", "Power Strip", "HDMI Cable", "Bluetooth Speaker", "Phone Case", "Screen Protector", "Wireless Mouse", "Keyboard"]
    };
  ...
  const getFilteredSuggestions = (category: string, haveItems: string[]) => {
    ...
    const filtered = allSuggestions.filter(suggestion => 
      !haveItems.some(haveItem => 
        suggestion.toLowerCase().includes(haveItem.toLowerCase()) || haveItem.toLowerCase().includes(suggestion.toLowerCase())
      )
    );
    ...
    <Card className={`${categoryColor}`}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <p className="text-sm font-medium text-gray-600">What do you have at home?</p>
          <div className="flex flex-wrap gap-2 mb-2">
            {haveItemOptions.map((item) => (
              <Button
                key={item}
                variant={selectedHaveItems.includes(item) ? "default" : "outline"}
                size="sm"
          ...
          {selectedHaveItems.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {selectedHaveItems.map((item) => (
                <div key={item} className="flex items-center gap-1 bg-blue-100 text-blue-800 px-2 py-1 rounded-md text-xs">
                  <span>{item}</span>
                  <Button
                    variant="ghost"
                    size="sm"
        ...
        {/* Manual Input */}
        <div className="space-y-2">
          <p className="text-sm font-medium text-gray-600">Add custom item:</p>
